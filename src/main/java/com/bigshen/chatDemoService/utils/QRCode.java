...
public class QRCode {
	...
	public static void main(String[] args) {
		...
		try  
        {  
            getLogoQRCode("https://www.baidu.com/", "小迪");
        }  
        catch (Exception e)  
        {  
            e.printStackTrace();  
     ...
     */
    public static File getLogoQRCode(String qrUrl,String productName)
    {
		File qrFile = new File("." + System.getProperty("file.separator") + new Date().getTime()+".jpg");

        String content = qrUrl;
        try
        {  
        	if ("linux".equals(os)) {
        		String cmd = "/usr/local/source/qrencode/bin/qrencode -o " + qrFile.getAbsolutePath() + " -s 6 " +"'"+qrUrl+"'";
			
				CmdUtil.executeCmd(cmd);
    		} else {
                BufferedImage bim = getQR_CODEBufferedImage(content, BarcodeFormat.QR_CODE, 250, 250, getDecodeHintType());
                qrFile= addLogo_QRCode(bim, new LogoConfig(), productName);
    		}
     ...
     */
    public static File addLogo_QRCode(BufferedImage bim, LogoConfig logoConfig, String productName)
    {
        try
        {
            /**
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
                    ...
                    BufferedImage outImage2 = new BufferedImage(410, 495, BufferedImage.TYPE_4BYTE_ABGR);
