...
public class ZooLock {
    ...
    public ZooLock() {
        try {
            zkClient = new ZooKeeper("127.0.0.1:2181", Integer.MAX_VALUE,
                    watchedEvent -> System.out.println("zookeeper init success..."));
        } catch (IOException e) {
            e.printStackTrace();
        }
        try {
    ...
    public boolean lock(final long timeout) {
        try {
            //新建一个临时有序节点
            String currentNode = zkClient.create(DEFAULT_LOCK_NODE + DEFAULT_COMP_NODE, new byte[0],
